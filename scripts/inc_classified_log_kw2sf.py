...
ELASTICSEARCH_URL_CLASSIFIED_KW = os.getenv("ELASTICSEARCH_URL_CLASSIFIED_LOG_KW")
...
try:
    cursor.execute("SELECT MAX(createdat) FROM classified_log_report_els")
    result = cursor.fetchone()
    print(result)
    last_day_in_db = result[0] if result and result[0] else '2024-10-01T00:00:00.230Z'  # Default to start date if no data
    logging.info(f"Last day in Snowflake database: {last_day_in_db}")
except Exception as e:
    logging.error(f"Failed to retrieve the last day from Snowflake: {e}")
    raise

# Convert last_day_in_db to an ISO 8601 string if it is a datetime object
...
# incremental_filter = {'range': {'created_at': {'gt': last_day_in_db}}} if last_day_in_db else {}

# Initialize max_updated_at variable to track the latest timestamp in the batch
max_updated_at = last_day_in_db if last_day_in_db else datetime.min  # Use a very old date if last_updated_at is None
...
while True:
    ...
    json_data = {
        ...
        'query': {
            'bool': {
                'filter': [
                    {'range': {'created_at': {'gte': last_day_in_db}}}  # Use the retrieved last day
                ]
            }
        },
        'sort': [{'created_at': 'asc'}, {'entity_id': 'asc'}],  # Sort order for pagination
    }
    logging.debug(f"Payload: {json_data}")
    ...
    insert_query = """
    INSERT INTO classified_log_report_els (
    ...
    logging.info(f"Inserted {len(rows_to_insert)} records into Snowflake.")

    # Set search_after for next iteration
    search_after_value = hits[-1]['sort']
    logging.info(f"Processed {len(hits)} records.")

# Update metadata table with the latest updated_at value
if max_updated_at:
    cursor.execute("""
        MERGE INTO etl_metadata AS target
        USING (SELECT 'classified_log_report_els' AS etl_name, %s AS last_updated_at) AS source
    ...
    """, (max_updated_at,))
    snowflake_conn.commit()
    logging.info(f"Updated metadata table with last_updated_at = {max_updated_at}.")
